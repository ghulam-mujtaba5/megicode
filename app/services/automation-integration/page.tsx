...
const service = servicesData.find(s => s.slug === "automation-integration");

const processSteps = [
  { title: "Process Mapping", desc: "Map workflows.", icon: "/IconSystem/requirements.svg" },
  { title: "Solution Design", desc: "Design automation.", icon: "/IconSystem/design.svg" },
  { title: "Implementation", desc: "Develop & deploy.", icon: "/IconSystem/development.svg" },
  { title: "Training & Handover", desc: "Train & document.", icon: "/IconSystem/deploy.svg" },
  { title: "Support & Optimization", desc: "Monitor & optimize.", icon: "/IconSystem/support.svg" }
];

const faqs = [
  { q: "Can you automate legacy systems?", a: "Yes, we can integrate and automate across modern and legacy platforms." },
  { q: "What tools do you use for automation?", a: "We use Python, Zapier, Make, Selenium, and custom scripts." },
  ...
  return (
    <main style={{ maxWidth: 900, margin: '0 auto', padding: '2.5rem 1rem', fontFamily: 'Inter, sans-serif' }}>
      ...
      <section style={{ display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap', marginBottom: 36 }}>
        <div style={{ flex: 1, minWidth: 260 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
            ...
            <span style={{ background: '#e3e6ea', color: '#4ea8ff', borderRadius: 8, padding: '0.3rem 1rem', fontWeight: 700, fontSize: '1.05rem' }}>Integration</span>
            <span style={{ background: '#e3e6ea', color: '#4ea8ff', borderRadius: 8, padding: '0.3rem 1rem', fontWeight: 700, fontSize: '1.05rem' }}>RPA</span>
          </div>
        </div>
        <div style={{ flex: 1, minWidth: 220, textAlign: 'center' }}>
          <img src="/data scrapping icon.svg" alt="Automation Illustration" style={{ width: 160, maxWidth: '100%', borderRadius: 24, boxShadow: '0 4px 24px #4ea8ff22' }} />
        </div>
      </section>
      ...
      <section style={{ margin: '48px 0 0 0', textAlign: 'center' }}>
        <a href="/contact" style={{ background: '#4ea8ff', color: '#fff', fontWeight: 800, fontSize: '1.18rem', borderRadius: 12, padding: '1rem 2.5rem', textDecoration: 'none', boxShadow: '0 2px 12px #4ea8ff33', transition: 'background 0.2s' }}>Start Your Automation Project</a>
        <div style={{ marginTop: 10, color: '#3a4a5d', fontSize: '1.01rem' }}>Ready to automate your business? Let’s talk about your vision.</div>
      </section>
    </main>
  );
}
