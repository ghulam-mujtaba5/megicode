...
export default function Breadcrumbs({ theme = 'light' }: { theme?: 'light' | 'dark' }) {
  ...
  // Words whose brand/industry casing differs from simple title case
  ...
  const items: BreadcrumbItem[] = segments.map((segment, index) => {
    ...
    // Formatting: replace hyphens with spaces, then case each word
    const name = segment
      .split('-')
      .map((word) => SPECIAL_CASING[word] ?? word.charAt(0).toUpperCase() + word.slice(1))
      .join(' ');
